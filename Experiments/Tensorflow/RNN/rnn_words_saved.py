...
with tf.Session() as session:
    ...
    pred = graph.get_tensor_by_name("network_out:0")

    # Loss and optimizer
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=y))
    optimizer = graph.get_operation_by_name("opt_rms") #tf.train.RMSPropOptimizer(learning_rate=learning_rate).minimize(cost)
